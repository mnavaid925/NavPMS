...
| TC-AUTH-05 | Tenant non-admin BLOCKED from rule CRUD | Logged in as `mgr_acme` (or any non-admin tenant user) | 1. Navigate to `/approvals/rules/`<br>2. Navigate to `/approvals/rules/create/`<br>3. Navigate to `/approvals/rules/1/edit/` | — | Each request either 403/redirect with an "admin only" message (per `TenantAdminRequiredMixin`). User must NOT see the rule list. | | |
...
| TC-CREATE-04 | Create rule with min > max amount | Same | 1. Visit `/approvals/rules/create/`<br>2. Fill Name `Bad range`, Min `1000`, Max `500`<br>3. Click **Save** | Min 1000, Max 500 | Form currently has NO cross-field validator. Expected: rule saves successfully BUT `matches()` will never return true (all docs fail both bounds). **CANDIDATE BUG — log as Low**: form should reject min > max. | | |
...
| TC-NEG-03 | Negative amounts | Rule create form | 1. Type `-100` in Min amount<br>2. Save | — | Form saves (no `MinValueValidator(0)` on the field per the model). **CANDIDATE BUG — log as Low**: negative thresholds will never match real reqs. | | |
...
| TC-NEG-16 | Comment length boundary | Pending task | 1. Open task<br>2. Type a 1000-character comment<br>3. Submit | — | Either form rejects with max-length error (CharField max=255 on `ApprovalTask.comment`) OR backend truncates. Whatever happens must NOT 500. Log if behavior is silently truncated. | | |
